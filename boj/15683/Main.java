...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int[] cctv : cctvs) {
			Queue<int[][]> newQ = new LinkedList<>(); // 가능한 새로운 모양을 저장하는 큐
			int type = nums[cctv[0]][cctv[1]]; // cctv 번호
			while(!q.isEmpty()) { // 가능한 모든 사무실 모양에 대하여
				for(int i=0; i<directions[type].length; i++) {
					int[][] board = new int[n][m]; // 큐에 있는 사무실 모양 복사할 배열
					for(int j=0; j<n; j++) {
						for(int k=0; k<m; k++) {
							board[j][k] = q.peek()[j][k];
						}
					}
					for(int j=0; j<directions[type][i].length; j++) {
						int r = cctv[0];
						int c = cctv[1];
						while(true) {
